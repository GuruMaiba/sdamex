...
class Signin extends Model
{
        ...
        if (!$error) {
            ...
            if ($user) {
                ...
                if ($user->status == UserStatus::INACTIVE) {
                    ...
                    $error = 'Ваша учётная запись не активирована!<br><a class="mailAgain" href="#">Отправить письмо повторно!</a>'; // DEBUG
                // Если пользователь имеет бан
                    ...
                    if (!$user->validatePassword($this->password)) {
                        ...
                        if ($count >= $allow) {
                            $ban = $user->getBan(); // получаем модель бана
    ...
    public function login($sub = 1)
    {
        ...
        if ( empty($fullStats[$sub]) ) {
            $fullStats[$sub] = ($sub != Yii::$app->params['listSubs']['MAIN']['id']) ? User::DEF_STAT : [];
            $user->statistics = json_encode($fullStats);
        }

        $user->update();

        return $user->login($this->rememberMe ? true : false);
    }
